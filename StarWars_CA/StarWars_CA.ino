...
void draw_type(int min_alive, int max_alive, uint16_t image){  
  ...
  for(int y = 0; y < HEIGHT; y++){
    ...
    for(int x = 0; x < WIDTH; x++){
      ...
      else if (self == DEATH_1) coll = ST7735_GREEN;
      else if (self == DEATH_2) coll = ST7735_YELLOW;
      
      col[((4*x)+(4*y)*FULLW)] = (uint8_t)(coll >> 8) & 0xFF;
      col[((4*x)+(4*y)*FULLW)+1] = (uint8_t)(coll) & 0xFF;
